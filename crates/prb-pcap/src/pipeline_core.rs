...
impl PipelineCore {
    /// Creates a new pipeline with the given TLS processor.
    ...
    pub fn process_packet(
        ...
        match &owned_normalized.transport {
            ...
            TransportInfo::Udp { src_port, dst_port } => {
                ...
                let event = create_udp_event(&owned_normalized, *src_port, *dst_port, origin);
    ...
    /// Process a reassembled TCP stream through TLS decryption and create a DebugEvent.
...
mod tests {
    ...
    fn test_pipeline_core_stats() {
        let mut core = PipelineCore::new(TlsStreamProcessor::new());
