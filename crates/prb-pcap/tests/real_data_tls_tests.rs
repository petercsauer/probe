...
fn test_tls13_decrypt_with_keylog() {
    ...
    assert!(
        pcap_path.exists(),
        "TLS 1.3 pcap file should exist: {pcap_path:?}"
    );
    assert!(
        keylog_path.exists(),
        "TLS 1.3 keylog file should exist: {keylog_path:?}"
    );
...
fn test_tls12_decrypt_with_keylog() {
    ...
    assert!(
        pcap_path.exists(),
        "TLS 1.2 pcap file should exist: {pcap_path:?}"
    );
    assert!(
        keylog_path.exists(),
        "TLS 1.2 keylog file should exist: {keylog_path:?}"
    );
...
fn test_tls_with_wrong_keylog() {
    ...
    assert!(
        ...
        "TLS 1.2 keylog file should exist: {wrong_keylog_path:?}"
    );
