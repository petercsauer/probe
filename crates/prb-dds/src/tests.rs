...
use prb_core::{
    DecodeContext, METADATA_KEY_DDS_DOMAIN_ID, METADATA_KEY_DDS_TOPIC_NAME, ProtocolDecoder,
    TransportKind,
};
...
fn test_rtps_message_parse() {
    ...
    message.extend_from_slice(&create_data_submessage(
        writer_entity,
        reader_entity,
        1,
        payload,
...
fn test_rtps_data_payload() {
    ...
    // Check metadata
    assert_eq!(
        event.metadata.get("dds.sequence_number"),
        Some(&"42".to_string())
    );
    assert!(event.metadata.contains_key("dds.writer_guid"));
...
fn test_rtps_discovery_topic_name() {
    ...
    let events = decoder
        .decode_stream(&discovery_msg, &ctx)
        .expect("decode should succeed");
    assert_eq!(
        events.len(),
        0,
        "Discovery messages should not generate events"
    );
...
fn test_rtps_domain_id_from_port() {
    ...
    let events = decoder
        .decode_stream(&message, &ctx_domain0)
        .expect("decode should succeed");
    assert_eq!(events.len(), 1);
    assert_eq!(
        events[0].metadata.get(METADATA_KEY_DDS_DOMAIN_ID),
        Some(&"0".to_string())
    );
    ...
    let events = decoder
        .decode_stream(&message, &ctx_domain1)
        .expect("decode should succeed");
    assert_eq!(events.len(), 1);
    assert_eq!(
        events[0].metadata.get(METADATA_KEY_DDS_DOMAIN_ID),
