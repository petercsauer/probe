...
pub struct App {
    ...
    ai_filter_rx: Option<mpsc::UnboundedReceiver<Result<String, String>>>,
...
impl App {
    pub fn new(
        ...
        if let Some(ref overrides) = config.tui.colors {
    ...
    /// Build conversations from events using the conversation engine.
    fn build_conversations(events: &[DebugEvent]) -> Option<prb_core::engine::ConversationSet> {
        if events.is_empty() {
            return None;
        }
        let mut engine = prb_core::engine::ConversationEngine::new();
        engine.register(Box::new(GrpcCorrelationStrategy));
        engine.register(Box::new(ZmqCorrelationStrategy));
        engine.register(Box::new(DdsCorrelationStrategy));

        match engine.build_conversations(events) {
            Ok(conv_set) => Some(conv_set),
            Err(e) => {
                tracing::warn!("Failed to build conversations: {}", e);
                None
            }
        }
    }
    ...
    /// Uses incremental filtering to only check new events since last filter.
    fn recompute_filter(&mut self) {
        if let Some(ref filter) = self.state.filter {
            // Use incremental filtering for better performance with streaming/live data
            self.state.filtered_indices = self.state.store.filter_indices_incremental(filter);
        } else {
    ...
    fn handle_key(&mut self, key: KeyEvent) -> bool {
        ...
        match key.code {
            ...
            KeyCode::Char('D') => {
                // Run anomaly detection on capture
                self.start_anomaly_detection();
                return false;
            }
            KeyCode::Char('P') => {
                // Identify protocol of selected event
                self.start_protocol_identification();
                return false;
            }
            KeyCode::Char('f') => {
                // Show which-key popup for quick-filter options
                let options = vec![
                    ('s', "filter by Source".to_string()),
                    ('d', "filter by Destination".to_string()),
                    ('p', "filter by Protocol".to_string()),
                    ('c', "filter by Conversation".to_string()),
                ];
                self.which_key_overlay = Some(WhichKeyOverlay::new('f', options));
                self.input_mode = InputMode::WhichKey;
    ...
    fn start_ai_filter_generation(&mut self, query: &str) {
        if self.state.store.is_empty() {
            self.set_status_message("No events to filter");
            return;
        }

        self.ai_filter_generating = true;
        self.ai_filter_generated = None;
        self.generate_filter_from_nl(query.to_string());
    }

    fn generate_filter_from_nl(&mut self, nl_query: String) {
        let config = self.config.ai.clone();
        let all_events = self.state.store.events().to_vec();

        // Create channel to receive result
        let (tx, rx) = mpsc::unbounded_channel();
        self.ai_filter_rx = Some(rx);

        // Spawn async task to generate filter
        tokio::spawn(async move {
            let context = crate::ai_smart::CaptureContext::build(&all_events);
            let result = crate::ai_smart::generate_filter(&nl_query, &context, &config).await;
    ...
    fn start_anomaly_detection(&mut self) {
        if self.state.store.is_empty() {
            self.set_status_message("No events to analyze");
            return;
        }
        ...
        // Show AI panel immediately with a loading message
        self.ai_panel_visible = true;
        self.ai_panel.clear();

        // Create channel to receive result
        let (tx, rx) = mpsc::unbounded_channel();
        self.anomaly_rx = Some(rx);

        // Spawn async task
        tokio::spawn(async move {
            let result = crate::ai_smart::detect_anomalies(&all_events, &config).await;
            let _ = tx.send(result);
        });

        self.set_status_message("Running anomaly detection...");
    }

    fn start_protocol_identification(&mut self) {
        if self.state.store.is_empty() {
            self.set_status_message("No events to analyze");
            return;
        }

        // Get payload from selected event
        if let Some(selected_idx) = self.state.selected_event {
            if let Some(&event_idx) = self.state.filtered_indices.get(selected_idx) {
                if let Some(event) = self.state.store.get(event_idx) {
                    let payload_bytes = match &event.payload {
                        Payload::Raw { raw } => raw.to_vec(),
                        Payload::Decoded { raw, .. } => raw.to_vec(),
                    };

                    if payload_bytes.is_empty() {
                        self.set_status_message("Selected event has no payload");
                        return;
                    }
                    ...
                    self.set_status_message("Identifying protocol...");
                } else {
                    self.set_status_message("No event selected");
                }
            } else {
    ...
    fn handle_command_palette_key(&mut self, key: KeyEvent) -> bool {
        match key.code {
            ...
            KeyCode::Enter => {
                // Check if input is a command (e.g., "theme dark")
                let input = self.command_palette.input.trim();
                if let Some(theme_name) = input.strip_prefix("theme ") {
                    // Theme switching command
                    let theme_name = theme_name.trim().to_string();
                    let mut new_theme = ThemeConfig::from_name(&theme_name);
                    // Apply any custom color overrides from config
                    if let Some(ref overrides) = self.config.tui.colors {
                        overrides.apply_to_theme(&mut new_theme);
                    }
                    self.theme = new_theme.clone();
                    self.config.tui.theme = theme_name.clone();
                    self.save_config();
                    self.set_status_message(&format!("Switched to {} theme", theme_name));
                    self.input_mode = InputMode::Normal;
                    return false;
                } else if input == "theme-editor" || input == "theme editor" {
                    // Open theme editor
                    ...
                    .or_else(|| input.strip_prefix("ai "))
                {
                    ...
                    if !query.is_empty() {
                        self.start_ai_filter_generation(&query);
                    } else {
                        self.set_status_message("Usage: /ai <query> (e.g., /ai show all errors)");
                    }
                    self.input_mode = InputMode::Normal;
                    return false;
                }
    ...
    fn handle_theme_editor_key(&mut self, key: KeyEvent) -> bool {
        let Some(ref mut editor) = self.theme_editor else {
            self.input_mode = InputMode::Normal;
            return false;
        };

        if editor.editing_color {
            // Editing a color value
            match key.code {
                KeyCode::Esc => {
                ...
                KeyCode::Enter => {
                    match editor.confirm_edit() {
                        Ok(()) => {
                            // Apply the updated theme
                            self.theme = editor.current_theme.clone();
                            self.set_status_message("Color updated");
                        }
                        Err(e) => {
                            self.set_status_message(&format!("Invalid color: {}", e));
                        }
                    }
                    false
                }
                KeyCode::Char(c) => {
                    editor.color_input.push(c);
                    false
                }
                KeyCode::Backspace => {
                    editor.color_input.pop();
                    false
                }
                _ => false,
            }
        } else {
            // Navigating color list
            match key.code {
                KeyCode::Esc => {
                ...
                KeyCode::Char('r') => {
                    // Reset to original theme (with overrides from config)
                    let mut theme = ThemeConfig::from_name(&self.config.tui.theme);
                    if let Some(ref overrides) = self.config.tui.colors {
                        overrides.apply_to_theme(&mut theme);
                    }
                    editor.reset_to_theme(&theme);
                    self.theme = theme;
                    self.set_status_message("Theme reset");
                    false
                }
                KeyCode::Char('s') => {
