...
pub struct App {
    ...
    theme_notification: Option<(String, Instant)>,
    config: Config,
    ...
    ring_buffer: Option<RingBuffer<prb_core::DebugEvent>>,
...
impl App {
    ...
    fn handle_goto_key(&mut self, key: KeyEvent) -> bool {
        match key.code {
            KeyCode::Enter => {
                let text = self.goto_input.value().to_string();
                if let Ok(event_id) = text.trim().parse::<usize>() {
                    // Find the event with this ID in filtered indices
                    if let Some(pos) = self.state.filtered_indices.iter().position(|&store_idx| {
                        self.state.store.get(store_idx).map(|e| e.id) == Some(event_id)
                    }) {
                        self.event_list.selected = pos;
                        self.event_list.ensure_visible(pos, 20); // ensure_visible with some context
                        self.state.selected_event = Some(pos);
                        self.goto_error = None;
                    } else {
                        self.goto_error = Some(format!("Event {} not found in current view", event_id));
                        return false;
                    }
                } else {
                    self.goto_error = Some("Invalid event ID".to_string());
                    return false;
                }
                self.input_mode = InputMode::Normal;
                self.focus = PaneId::EventList;
